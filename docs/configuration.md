...
`output_root` and `boltz_cache_dir` default to `""` so a host profile can fill
them. They are checked by `RunConfig.validate_paths()` at the run boundary
(inside `Runner`), which raises if either is still empty. See
[multi_user.md](multi_user.md) for host profiles.
