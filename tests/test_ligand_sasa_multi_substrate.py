...
def test_sasa_together_unexpected_substrate_error_does_not_leak_unsuffixed_keys(tmp_path, monkeypatch):
    """If a substrate raises an UNEXPECTED error mid-loop, its failure must stay
    isolated as a suffixed _s{i} None block and must NOT fall through to the
    outer handler and write UNSUFFIXED keys mixing with the good substrate."""
    (tmp_path / "chai_0.pdb").write_text("dummy")
    df = pd.DataFrame({"Entry": ["P1"], "docked_structure": ["chai_0"],
                       "substrate_smiles": ["CCO.c1ccncc1"]})
    ...
    assert len(out) == 1
    assert out.loc[0, "buried_sasa_s0"] == 100.0 - 40.0
    assert out.loc[0, "buried_sasa_s1"] is None
    assert "buried_sasa" not in cols
    assert "sasa_ligand_alone" not in cols
